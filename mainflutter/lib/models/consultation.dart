...
class Consultation {
  ...
  factory Consultation.fromJson(Map<String, dynamic> json) {
    final consultationDate = json['consultationDate'] != null 
        ? DateTime.parse(json['consultationDate']) 
        : DateTime.now();
    
    // Tính endTime từ consultationDate nếu không có trong json
    final endTime = json['endTime'] != null 
        ? DateTime.parse(json['endTime'])
        : consultationDate.add(const Duration(minutes: 30));
  ...
  // Lấy ra thông tin thời gian kết thúc ở định dạng dễ đọc
  ...
  String getFullFormattedDateTime() {
    return '${getFormattedDate()} ${getFormattedTime()} - ${getFormattedEndTime()}';
  }
