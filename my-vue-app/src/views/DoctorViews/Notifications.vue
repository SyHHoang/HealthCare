...
const markAsRead = async (notificationId) => {
  try {
    await axiosInstance.patch(`/api/notifications/doctor/${notificationId}/read`);
    const notification = notifications.value.find(n => n._id === notificationId);
    if (notification) {
      notification.read = true;
      // Cập nhật số thông báo chưa đọc trong layout
      const event = new CustomEvent('update-unread-notifications', {
        detail: { count: notifications.value.filter(n => !n.read).length }
      });
...
const deleteNotification = async (notificationId) => {
  try {
    await axiosInstance.delete(`/api/notifications/doctor/${notificationId}`);
